...
| 缺证项 | 需要走的入口 | 当前状态 |
| --- | --- | --- |
| `.stmcx` / MotorControl 配置截图 | MCSDK / MotorControl Configuration Packet | Blocked. 仍没有真实 `.stmcx` 或 MotorControl 配置页截图。 |
...
Packet A remains `Blocked`. The new checklist only defines the next acceptable
capture: a real `.stmcx`, a MotorControl / Workbench configuration screenshot,
or an exact reproducible launcher path plus captured version/config screen.
...
`Not allowed` because Packet A remains blocked. If Packet A later exists, a
generated MCSDK project can be treated only as no-power build evidence. Build
...
decision remains: P2 is in progress, Packet A is `Blocked`, generated-project
