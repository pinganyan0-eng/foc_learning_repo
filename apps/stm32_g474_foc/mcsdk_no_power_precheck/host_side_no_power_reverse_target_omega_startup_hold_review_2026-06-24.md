...
fixture chain. It protects the startup / unlocked boundary when a negative
`target_omega_e_rad_s` command arrives while the frontend is not locked:
...
-> hold_when_unlocked freezes the speed-loop target at +20.0 while unlocked
-> effective iq remains zero while unlocked
-> only after lock does the rate-limited command ramp begin
...
the existing `SensorlessSpeedLoopConfig.hold_when_unlocked`,
...
- `tests/test_foc_sensorless_frontend.py`
  ...
  - starts from `mode = startup`, `locked = false`, and
    `speed_loop_state.target_omega_e_rad_s = 20.0`;
  - commands `target_omega_e_rad_s = -20.0` with
    `target_omega_rate_limit_e_rad_s2 = 50.0`, `hold_when_unlocked = true`,
    and `dt_s = 0.1`;
  - verifies that the first two startup / unlocked steps keep the speed-loop
    target frozen at `20.0`, keep effective current at zero, and only begin
    ramping after lock.
- `tests/fixtures/foc_sensorless_frontend_vectors.json`
  - adds
    `reverse_target_omega_remains_frozen_until_lock_not_startup_strategy`;
  - records the same startup-hold replay for fixture-driven regression tests.
...
command can move only 5 rad/s per locked step. Because the frontend is still
startup / unlocked for the first two samples, those samples do not move the
target toward the negative command. The ramp starts only after lock is
confirmed.

Protected rows:

```text
speed_loop_target_omega: [20.0, 20.0, 15.0, 10.0, 5.0]
speed_loop_target_iq:    [0.0, 0.0, 2.0, 2.0, 2.0]
speed_loop_pi_integrator:[0.5, 0.5, 0.5, 0.5, 0.5]
effective_target_iq:     [0.0, 0.0, 1.5, 1.5, 1.5]
command_reasons:         [unlocked_current_limit, unlocked_current_limit, tracking_command, tracking_command, tracking_command]
q_axis_integrator:       [0.0, 0.0, 1.5, 3.0, 4.5]
locked:                  [false, false, true, true, true]
lock_candidate_count:    [0, 1, 2, 2, 2]
final_target_omega:      5.0
...
```text
target_omega_q15: [32767, 32767, 24576, 16384, 8192]
measured_omega_q15: [1638, 3277, -16384, -16384, -16384]
requested_iq_q15: [0, 0, 32767, 32767, 32767]
effective_iq_q15: [0, 0, 24576, 24576, 24576]
locked: [false, false, true, true, true]
limited: [false, false, true, true, true]
reason: [unlocked_current_limit, unlocked_current_limit, tracking_command, tracking_command, tracking_command]
```
...
Read-only helper Boole reviewed only the filtered current snapshot, active
task, priority matrix, frontend fixture, and bridge fixture slices. Boole
identified this startup / unlocked reverse target hold fixture as the smallest
remaining P0 gap because it closes the ambiguity between locked reverse
commands and reverse open-loop startup strategy. The main agent kept all repo
writes in the owner path and used the helper digest only as
