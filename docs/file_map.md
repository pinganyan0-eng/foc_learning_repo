...
| `fact_registry` | `docs/00_project_truth/fact_registry.jsonl` | Structured high-risk fact registry for no-power boundaries, 24 V, CN3/CN8 alias discipline, Gate/OUTx/BOOTx/Vgs limits, BKIN, B1 STOP latch, MCSDK/FOC trust limits, and internet verification rules. |
| `fact_registry_schema` | `docs/00_project_truth/fact_registry.schema.json` | JSON schema for one `fact_registry.jsonl` record; checked by local contracts without adding runtime dependencies. |
| `retrieval_eval_queries` | `retrieval_eval/queries.json` | Regression cases for dual-teacher guard, PCB2 Hall route, DMM pending/no-power boundary, current nFAULT fault-tree safety boundary, ACTIVE_TASK review lifecycle, ESP32 real-time boundary, automation no-write boundary, learning loop, closeout checklist, repo-maintenance DoD, project Skill v2 router, subagent protocol, and dangerous claim scan coverage. |
| `search_local_v2_eval` | `tools/search_local_v2.py --eval` | Source-finding regression check; passing it is not hardware validation. |
| `ask_local_fact_registry` | `tools/ask_local.py` | Offline TF-IDF local Q&A plus fact registry matching, task-type judgment, risk level, internet-required flag, structured fact hits, and forbidden-action reminders. |
| `ai_architecture_eval_schema` | `evals/ai_architecture_eval.schema.json` | JSON schema for each local AI architecture eval case; covers must-include, must-not-include, risk expectation, and internet-required expectation fields. |
| `hardware_safety_eval` | `evals/hardware_safety_eval.jsonl` | Offline eval cases for 24 V, Gate/PWM, OUTx/BOOTx/Vgs, motor, Hall, nFAULT, B1 STOP latch, and FOC runtime safety boundaries. |
| `internet_required_eval` | `evals/internet_required_eval.jsonl` | Offline eval cases for MCSDK, CubeMX, CubeIDE plugin, Codex, competition rules, stock, substitute-part, and high-risk parameter internet-verification requirements. |
| `fact_conflict_eval` | `evals/fact_conflict_eval.jsonl` | Offline eval cases for CN3/CN8 alias discipline, PB3 LIN1/Hall conflict, generated Hall route mismatch, local eval limits, project_context priority, and BKIN protection claims. |
| `ai_architecture_evals` | `tools/run_ai_architecture_evals.py` | Local safety and internet-required regression runner for `hardware_safety_eval`, `internet_required_eval`, and `fact_conflict_eval`; passing it is AI architecture evidence only, not hardware validation. |
...
| `subagent_protocol_stub` | `workflow/SUBAGENT_PROTOCOL.md` | Compatibility entrypoint only for startup routing; the sole authoritative subagent protocol remains `docs/00_project_truth/ai_architecture.md` under `## Subagent Communication Protocol`. |
