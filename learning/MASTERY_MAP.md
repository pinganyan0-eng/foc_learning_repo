...
| Software Hall state-machine table classification | User correctly classified `100 -> 110`, `100 -> 101`, `100 -> 011`, `000`, `111`, and later the ChatGPT follow-up cases `110 -> 010`, `010 -> 110`, `111`, `60 / 7`, and `PB3=LIN1` | L4 for table-level no-power algorithm classification; L2-L3 for electrical/mechanical angle conversion | Move from concept tables to adapter processing-order review and pseudocode boundary. Do not treat this as firmware, GPIO/EXTI, MCSDK Hall, Gate PWM, power-board, motor, or Hall closed-loop evidence. |
| Software Hall adapter processing-order transfer | User independently traced the mixed sequence `(1000,100), (1600,100), (2200,110), (2210,010), (3000,111), (3800,011)` with threshold `50`, including baseline, repeat, accepted adjacent forward candidate, rejected bounce candidate, illegal state, and abnormal jump from the last trusted state | L4 for no-power mixed-sequence classification | Move next to no-power pseudocode and firmware-entry boundary review. Do not treat this as firmware, GPIO/EXTI, MCSDK Hall, Gate PWM, power-board, motor, or Hall closed-loop evidence. |

## Active Transfer Gaps

| Gap | Why It Still Matters | Next Proof Needed |
| --- | --- | --- |
| Software Hall adapter firmware-transfer boundary | The no-power processing order is now L4 at mixed-sequence level, but firmware transfer still has higher-risk boundaries: ISR work must stay minimal, trusted-state updates must be explicit, debug outputs must stay low-frequency, and MCSDK writes remain forbidden without an accepted feedback-object design. | Before any software Hall firmware implementation, walk through pseudocode and mark ISR-only capture, low-priority classification, rejected states, trusted-state update points, debug-only outputs, and forbidden MCSDK writes. |
