...
| completed 2026-05-30 | Hall reverse-adjacent and PB3 route constraint | Classify `110 -> 010`, `010 -> 110`, `111`, calculate `60 / 7`, and explain why `PB3=LIN1` is not Hall. | `learning/review_items/2026-05-30_hall_state_machine_chatgpt_followup.md` | done |
| before software Hall firmware | MCSDK Hall feedback destination classification | Classify `SPD_GetElAngle(...)`, `SPD_GetAvrgMecSpeedUnit(...)`, raw `PA0/PA1/PB4`, `direction_candidate`, and `speed_candidate` by destination: current loop, speed loop, debug log, or nowhere, and explain why `HALL_M1` is not the three Hall pins. | PR #5 / 2026-06-01 MCSDK Hall feedback review | open |
