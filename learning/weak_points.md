...
| WP-029 | NUCLEO DMA receive size/index | L1 | User initially answered that `Size = 6` should process 5 bytes, mixing up byte count with final zero-based index. User corrected the rule on the follow-up `Size = 3` check. | Reinforce the rule with small arrays before real HAL callback code: `Size` is count, loop condition is `i < Size`, and the final index is `Size - 1`. | Given `Size = N`, state the processed indices, loop condition, and final index. | open |
