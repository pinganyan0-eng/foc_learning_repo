...
| --- | --- | --- | --- | --- | --- | --- |
| WP-030 | Software Hall adapter processing order | L1 | User can classify individual Hall transition rows but answered `我不知道啊` when asked to restate raw read -> illegal-state check -> first-valid check -> repeated-state check -> bounce/timing check -> forward/reverse adjacent check -> abnormal-jump count. | Use the Chinese-first processing-order card; reduce the target to one sentence before asking for code or firmware boundaries. | User says: first read `PA0/PA1/PB4`, reject `000/111`, first valid state is only a baseline, repeated states do not count as edges, too-fast changes are bounce candidates, adjacent changes define direction candidates, non-adjacent legal changes are abnormal jumps. | open |
