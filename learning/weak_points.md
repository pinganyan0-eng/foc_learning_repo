...
| WP-024 | NUCLEO STOP command idempotence | L1 | User initially treated IDLE + STOP as a state change instead of an accepted no-op. | Teach idempotent safe commands: repeated STOP should leave the app in IDLE while still reporting an OK/already-idle response. | Explain why IDLE + STOP and RUN_SIM + STOP are both accepted, but only one changes state. | open |
...
| WP-028 | NUCLEO command matching versus side effect | L1 | Need distinguish condition checks from side effects: `if`/`strcmp` chooses a branch, but assignments such as `*app_mode = ...` change state. | Label command branches with two marks: match condition and side-effect lines. | Identify the line that changes state in ARM and the line that only prints output in PING. | open |
