...
## Codex Concept-Only Guard

当用户问理论、概念、"我不懂"、"教我"、"还要学什么"、或类似纯概念问题时，Codex 先判断这是不是 ChatGPT 主讲场景。

- 如果不需要读取/修改仓库文件、不需要运行命令、不需要处理构建输出、测试、日志、截图、学习记录或 GitHub，Codex 不讲完整理论课。
- Codex 应输出一段可直接发给 ChatGPT 的具体教学 prompt 或任务包。
- Codex 应说明用户学完后带回什么：答案、表格、总结、截图或交接包。
- 用户带回结果后，Codex 负责审查、记录证据、更新学习闭环，并决定下一步工程动作。
- 如果 ChatGPT 有 GitHub 写入能力，Codex 可以要求 ChatGPT 直接开学习证据 PR。这个 PR 是教学交接物，只有经过 Codex 拉取、审查、验证、同步后，才算进入本地项目事实。
- 如果任务进入真实文件、代码、构建、测试、证据记录、PR/GitHub、硬件安全边界或项目状态判断，Codex 负责执行，不把当前 Codex 侧工程活推给 ChatGPT。
...
- 只更新 `learning/` 和必要的教学工作流文件；不要改固件、Workbench/MCSDK 生成代码、硬件证据、阶段闸门或项目状态结论。
- 新建分支，不直接合并 `master`。
- 开 PR，标题建议：`learning: record STM32 session YYYY-MM-DD`。
- PR 描述必须说明本次课证据、弱点更新、下次复习点和未验证事项。
- PR 描述必须写清楚证据等级，例如 L1/L2 概念理解、L3 练习表、L4 可复述/可迁移；不能把概念课写成编译、上电、Hall 闭环、Gate PWM、Motor Profiler 或电机验证完成。
- Codex 后续负责同步分支、审查 PR、运行必要检查、合并或要求修改，并把合并结果写入 `CURRENT_STATUS.md` / evidence register。
